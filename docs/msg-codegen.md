...
rclsharp は ROS 2 の `.msg` から、CDR 互換な C# 型 (`struct`) と
シリアライザ (`ICdrSerializer<T>`) のペアを生成する仕組みを持つ。
IL2CPP / AOT 互換のため**すべてコンパイル時生成**で、ランタイム生成は行わない。

生成ロジックは依存ゼロの netstandard2.0 ライブラリ `Rclsharp.MsgGen` に集約され、
以下 2 つのフロントエンドから利用できる。

| フロントエンド | プロジェクト | 用途 |
| --- | --- | --- |
| CLI | `tools/rclsharp-genmsg` | 標準 msg の保守、Unity/任意プロジェクトへの生成 |
| Source Generator | `src/Rclsharp.SourceGenerator` | .NET プロジェクトでのコンパイル時透過生成 |
...
`Rclsharp.MsgGen.TypeMapping.TypeNameResolver` が決定する。
...
- 名前空間は `_msgs` を畳む: `std_msgs`→`Rclsharp.Msgs.Std`、
  `geometry_msgs`→`Rclsharp.Msgs.Geometry`、`builtin_interfaces`→`Rclsharp.Msgs.BuiltinInterfaces`
...
dotnet run --project tools/rclsharp-genmsg -- --input msgs --output src/rclsharp/Msgs --check
```

`src/rclsharp/Msgs/` の標準型はこの CLI の生成物であり、入力は `msgs/` にある。
...
```xml
<ItemGroup>
  <ProjectReference Include="path/to/src/rclsharp/rclsharp.csproj" />
  <ProjectReference Include="path/to/src/Rclsharp.SourceGenerator/Rclsharp.SourceGenerator.csproj"
                    OutputItemType="Analyzer" ReferenceOutputAssembly="false" />
</ItemGroup>

<ItemGroup>
  <AdditionalFiles Include="msgs\**\*.msg" RclsharpMsgPackage="sample_msgs" />
  <!-- パッケージ名メタデータを生成器へ渡すための宣言 -->
  <CompilerVisibleItemMetadata Include="AdditionalFiles" MetadataName="RclsharpMsgPackage" />
</ItemGroup>
```

パッケージ名は `RclsharpMsgPackage` メタデータを優先し、無ければパス
...
2. .NET SDK が入った環境で CLI を実行し、Assets 配下へ `.cs` を生成する。
   ...
   ```sh
   dotnet run --project /path/to/rclsharp/tools/rclsharp-genmsg -- \
     --input  Assets/Msgs \
     --output Assets/Scripts/GeneratedMsgs
   ```

3. Unity に戻ると生成 `.cs` がインポートされコンパイルされる。
   生成型は rclsharp パッケージの `Rclsharp.Cdr` / 既存 `Rclsharp.Msgs.*` を参照する。

生成 `.cs` は Assets 配下に置かれるため `.meta` は Unity が自動付与する。
標準 msg (`std_msgs` 等) は rclsharp パッケージに同梱済みなので、
...
```
Rclsharp.MsgGen (netstandard2.0, 依存ゼロ)
...
tools/rclsharp-genmsg          Core を呼ぶ CLI (ファイル入出力)
src/Rclsharp.SourceGenerator   Core をソースリンクで取り込む IIncrementalGenerator
```

生成される `Serialize`/`Deserialize`/`GetSerializedSize` は既存手書き型と
CDR バイト列が一致する。回帰は `tests/rclsharp.Tests/Msgs/*`
