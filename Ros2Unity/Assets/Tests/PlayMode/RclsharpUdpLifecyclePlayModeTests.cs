...
namespace Rclsharp.UnityPlayMode.Tests
{
    public sealed class RclsharpUdpLifecyclePlayModeTests
    {
        [UnityTest]
        public IEnumerator MonoBehaviour_lifecycleで実UDP_pubsubを開始停止できる()
        {
            var go = new GameObject("rclsharp-udp-lifecycle-probe");
            var probe = go.AddComponent<RclsharpUdpLifecycleProbe>();
            ...
            yield return null;
            Assert.GreaterOrEqual(RclsharpUdpLifecycleProbe.DestroyCount, 1);
        }

        private static IEnumerator PublishUntilReceived(RclsharpUdpLifecycleProbe probe, string value)
        {
    ...
    public sealed class RclsharpUdpLifecycleProbe : MonoBehaviour
    {
